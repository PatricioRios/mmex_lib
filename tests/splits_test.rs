mod common;
use mmex_lib::domain::transactions::{Transaction, TransactionId, TransactionCode, TransactionStatus, SplitTransaction};
...
fn test_transaction_splits_integration() {
    ...
    let tx = Transaction {
        id: TransactionId(0), account_id: AccountId(1), to_account_id: None, payee_id: PayeeId(1), trans_code: TransactionCode::Withdrawal, amount: Money(dec!(100.0)), status: TransactionStatus::None, transaction_number: None, notes: None, category_id: None, date: None, to_amount: None,
    };
    let created_tx = service.create_transaction(&tx).unwrap();
    
    // 2. Añadir Desgloses (Splits)
    let split1 = SplitTransaction {
        id: 0, transaction_id: created_tx.id, category_id: Some(CategoryId(10)), amount: Money(dec!(60.0)), notes: Some("Comida".into()),
    };
    let split2 = SplitTransaction {
        id: 0, transaction_id: created_tx.id, category_id: Some(CategoryId(11)), amount: Money(dec!(40.0)), notes: Some("Bebida".into()),
    };
