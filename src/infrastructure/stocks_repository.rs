...
impl<'a, E: DbExecutor> StockRepository for SqlStockRepository<'a, E> {
    fn find_all(&self) -> Result<Vec<Stock>, MmexError> {
        ...
        self.executor.query_map_ext(&sql, [], |row| StockMapper::map_row(row))
    }
    ...
    fn insert(&self, s: &Stock) -> Result<Stock, MmexError> {
        ...
        self.executor.execute_ext(sql, (
            s.held_at, s.purchase_date.to_string(), &s.name, &s.symbol, 
            s.num_shares.to_string(), s.purchase_price.0.to_string(), &s.notes, 
            s.current_price.0.to_string(), s.value.0.to_string(), s.commission.0.to_string()
        ))?;
    ...
    fn update(&self, s: &Stock) -> Result<(), MmexError> {
    ...
    fn delete(&self, id: StockId) -> Result<(), MmexError> {
        self.executor.execute_ext("DELETE FROM STOCK_V1 WHERE STOCKID = ?", [id.0])?;
        Ok(())
    }
}
