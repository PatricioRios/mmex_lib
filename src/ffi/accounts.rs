...
use crate::domain::accounts::{Account, AccountBalance};
...
impl AccountManager {
    ...
    pub fn get_all(&self) -> Result<Vec<Account>, MmexError> {
        let ctx = self
            .context
            .lock()
    ...
    pub fn get_by_id(&self, id: i64) -> Result<Option<Account>, MmexError> {
        let ctx = self
            .context
            .lock()
            .map_err(|e| MmexError::Internal(e.to_string()))?;
    ...
    pub fn create(&self, account: Account) -> Result<Account, MmexError> {
        let ctx = self
            .context
            .lock()
            .map_err(|e| MmexError::Internal(e.to_string()))?;
    ...
    pub fn update(&self, account: Account) -> Result<(), MmexError> {
        let ctx = self
            .context
            .lock()
            .map_err(|e| MmexError::Internal(e.to_string()))?;
    ...
    pub fn get_balance(&self, account_id: i64) -> Result<AccountBalance, MmexError> {
        let ctx = self
            .context
            .lock()
            .map_err(|e| MmexError::Internal(e.to_string()))?;
